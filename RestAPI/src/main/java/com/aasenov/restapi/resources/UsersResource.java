 ...
 */
public class UsersResource extends WadlServerResource {
    /**
     * Logger instance.
     */
    private static Logger sLog = Logger.getLogger(UsersResource.class);
    ...
    public Representation authenticateUser(Representation entity) throws ResourceException {
        ...
        if (authenticated) {
            ...
            try {
                return MapperProvider.getMapper(getRequest()).getRepresentation(user);
            } catch (MapperException e) {
                setStatus(Status.SERVER_ERROR_INSUFFICIENT_STORAGE);
                return new StringRepresentation("Unable to deserialize user object.", MediaType.TEXT_PLAIN);
            }
        } else {
    ...
    public Representation registerUser(Representation entity) throws ResourceException {
        ...
        if (UserManager.getInstance().createUser(new UserItem(userName, userPass, userMail))) {
            ...
            sLog.info(message);
            try {
                return MapperProvider.getMapper(getRequest()).getRepresentation(
                        UserManager.getInstance().getUser(userMail));
            } catch (MapperException e) {
                setStatus(Status.SERVER_ERROR_INSUFFICIENT_STORAGE);
                return new StringRepresentation("Unable to deserialize user object.", MediaType.TEXT_PLAIN);
            }
